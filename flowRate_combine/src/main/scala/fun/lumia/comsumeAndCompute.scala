...
import fun.lumia.utils.DateUtils.{formatFhEndLong, formatFhStartLong, formatTimeTemp, tranTimeToLong, tranTimeZte, tranTimeb2bfh, tranTimeb2bhy}
...
object comsumeAndCompute extends SparkTool {
  ...
  override def run(args: Array[String]): Unit = {
    ...
    ssc.checkpoint(ConstantInCombine.SPARK_STREAMING_CHECKPOINT_PATH + "/" + ConstantInCombine.COMBINE_CHECKPOINT_PATH_NAME)

    val fonsviewDS = createKafkaSC(ssc, ConstantInCombine.FONSVIEW_KAFKA_B2B_TOPIC_NAME)

    val hanYanDS = createKafkaSC(ssc, ConstantInCombine.HANG_YAN_KAFKA_TOPIC_NAME)

    val zteDS = createKafkaSC(ssc, ConstantInCombine.ZTE_KAFKA_B2B_TOPIC_NAME)
    ...
    unionSS.foreachRDD(rdd => {
      ...
      //      finalDF.show(10)
      finalDF.write.mode(SaveMode.Overwrite).jdbc(ConstantInCombine.MYSQL_URL, ConstantInCombine.MYSQL_DATABLE_B2B_KPI_DWS, prop)
      finalDF.foreachPartition(iter => {
        ...
        val dbTable = ConstantInCombine.MYSQL_DATABLE_B2B_KPI
        val sql =
          s"""
